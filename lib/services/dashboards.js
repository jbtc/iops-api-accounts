...
const sortWidgets = widgets => widgets.sort((widget1, widget2) => {
  return (widget1.position || 0) - (widget2.position || 0);
});
...
export default {
  ...
  async find(criteria) {
    const dashboards = await COLLECTION.find(criteria).map(dashboard => {
  ...
  async findOne(criteria) {
    ...
    dashboard.widgets = sortWidgets(dashboard.widgets);
